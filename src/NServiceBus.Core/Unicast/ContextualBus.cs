    ...
    class ContextualBus : IBus, IManageMessageHeaders
    {
        ...
        void SetupHeaderActions()
        {
            SetHeaderAction = (message, key, value) =>
            {
                ...
                if (!context.TryGet("NServiceBus.OutgoingHeaders", out outgoingHeaders))
                {
                    outgoingHeaders = new Dictionary<object, Dictionary<string, string>>();

                    context.Set("NServiceBus.OutgoingHeaders", outgoingHeaders);
                }
            ...
            GetHeaderAction = (message, key) =>
            {
                if (message == ExtensionMethods.CurrentMessageBeingHandled)
                {
                    ...
                    if (context.TryGet(out messageBeingReceived))
                    {
                        string value;
        ...
        public void HandleCurrentMessageLater()
        {
            ...
            ((HandlingContext)context).DoNotInvokeAnyMoreHandlers();
        }
