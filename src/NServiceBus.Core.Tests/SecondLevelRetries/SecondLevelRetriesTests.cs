namespace NServiceBus.Core.Tests.SecondLevelRetries
{
    ...
    public class SecondLevelRetriesTests
    {
        ...
        IncomingContext CreateContext(string messageId, int currentRetryCount)
        {
            var context = new IncomingContext(null);

            context.SetPublicReceiveAddress("test-address-for-this-pipeline");
            context.Set(IncomingContext.IncomingPhysicalMessageKey, new TransportMessage(messageId, new Dictionary<string, string> { { Headers.Retries, currentRetryCount.ToString() } }));

            return context;
        }
    }

    class FakePolicy : RetryPolicy
    {
